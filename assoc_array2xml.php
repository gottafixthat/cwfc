...
function array2xml($array, $name = "array") {
//global $text;
...
function array_transform($array, $nestlevel = 1)
{
    //global $array_text;
    foreach($array as $key => $value) {
        if(!is_array($value)) {
            $this->text .= str_repeat(" ", $nestlevel * 4) . "<$key>$value</$key>\n";
        } else {
            $this->text.=str_repeat(" ", $nestlevel * 4) . "<$key>\n";
            $this->array_transform($value, $nestlevel + 1);
            $this->text.=str_repeat(" ", $nestlevel * 4) . "</$key>\n";
        }
    }
    return ""; //$array_text;
